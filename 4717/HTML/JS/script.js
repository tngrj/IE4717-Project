...
function register() {
    ...
    if (adduserinfo.addemail === "" || adduserinfo.addpassword === "" || adduserinfo.addlastname === "" || adduserinfo.addfirstname === "" || adduserinfo.addbirthday === "" ){
        return;
    }
    ...
    request.onload = function () {
        ...
        for (var i = 0; i < response.length; i++) {  
            ...
            tbodyuAppt.innerHTML += 
                ...
                '<td colspan="3">' + response[i].time + '</td>' +
                '<td colspan="3">' + response[i].doctor + '</td>' +
                '<td colspan="5">' + response[i].appointmentType + '</td>' +
                '<td colspan="3" class="centered-t">' +
                    '<button type="submit" style="margin-right: 20px;" id="editAppt" onclick="editAppt(' + response[i].apptId + ')">Edit</button>' +
                    '<button type="submit" id="cancelAppt" onclick="cancelAppt(' + response[i].apptId + ')">Cancel</button>' +
                '</td>' +
            '</tr>';
        }
    };
